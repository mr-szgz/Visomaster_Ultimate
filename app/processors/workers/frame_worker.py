...
class FrameWorker(threading.Thread):
    def __init__(self, frame, main_window: 'MainWindow', frame_number, frame_queue, is_single_frame=False, is_batch_processing=False):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            # For batch processing, we stop here and just let the caller access self.frame
            # For UI operations, we proceed to create pixmaps and emit signals.
            ...
            # Check if playback is complete (not relevant for batch processing)
    ...
    def process_frame(self):
        ...
        # --- CRITICAL CHANGE FOR BATCH PROCESSING ---
        # If in batch mode, we ALWAYS run detection for the current frame.
        # Otherwise, we use the globally stored target faces.
        if self.is_batch_processing:
            # We need a reference set of parameters for detection, even if they're not used for swapping.
            # We'll take it from the first available target face that was used for initialization.
            if not self.target_faces:
                print("Warning: Batch processing started but no initial target faces found. Skipping frame.")
                if img_x < 512 or img_y < 512:
                    tscale_back = v2.Resize((img_y, img_x), antialias=False)
                    img = tscale_back(img)
                img = img.permute(1,2,0).cpu().numpy()
                return img[..., ::-1] # Return original frame if no settings are available
            
            # Run detection on the current frame
            bboxes, kpss_5, kpss = self.models_processor.run_detect(img, control['DetectorModelSelection'], max_num=control['MaxFacesToDetectSlider'], score=control['DetectorScoreSlider']/100.0, input_size=(512, 512), use_landmark_detection=use_landmark_detection, landmark_detect_mode=landmark_detect_mode, landmark_score=control["LandmarkDetectScoreSlider"]/100.0, from_points=from_points, rotation_angles=[0] if not control["AutoRotationToggle"] else [0, 90, 180, 270])
            
            # Create a temporary list of detected faces for this frame only
            per_frame_target_faces = []
            if len(kpss_5)>0:
                for i in range(kpss_5.shape[0]):
                    face_kps_5 = kpss_5[i]
                    face_emb, _ = self.models_processor.run_recognize_direct(img, face_kps_5, control['SimilarityTypeSelection'], control['RecognitionModelSelection'])
                    per_frame_target_faces.append({'kps_5': face_kps_5, 'kps_all': kpss[i], 'embedding': face_emb, 'bbox': bboxes[i]})
            
            det_faces_data = per_frame_target_faces
            target_faces_to_use = self.target_faces # Use the initially detected faces as the template for settings
        else:
            # Standard logic for single frame/video playback
            bboxes, kpss_5, kpss = self.models_processor.run_detect(img, control['DetectorModelSelection'], max_num=control['MaxFacesToDetectSlider'], score=control['DetectorScoreSlider']/100.0, input_size=(512, 512), use_landmark_detection=use_landmark_detection, landmark_detect_mode=landmark_detect_mode, landmark_score=control["LandmarkDetectScoreSlider"]/100.0, from_points=from_points, rotation_angles=[0] if not control["AutoRotationToggle"] else [0, 90, 180, 270])
            det_faces_data = []
            if len(kpss_5)>0:
                for i in range(kpss_5.shape[0]):
                    ...
                    face_emb, _ = self.models_processor.run_recognize_direct(img, face_kps_5, control['SimilarityTypeSelection'], control['RecognitionModelSelection'])
                    det_faces_data.append({'kps_5': face_kps_5, 'kps_all': face_kps_all, 'embedding': face_emb, 'bbox': bboxes[i]})
            target_faces_to_use = self.target_faces

        compare_mode = self.is_view_face_mask or self.is_view_face_compare
        
        if det_faces_data:
            for i, fface in enumerate(det_faces_data):
                    for _, target_face in target_faces_to_use.items():
