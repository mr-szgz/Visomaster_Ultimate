...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def check_clipboard(self):
        ...
        if mime_data.hasUrls():
            ...
            if urls:
                ...
                for url in urls:
                    if url.isLocalFile():
                        potential_files.append(url.toLocalFile())
        elif mime_data.hasText():
            new_clipboard_content = mime_data.text()
            if new_clipboard_content:
                potential_files = new_clipboard_content.splitlines()
        ...
        if new_clipboard_content and new_clipboard_content != self.last_clipboard_content:
            ...
            if valid_files:
                is_busy = (self.batch_worker and self.batch_worker.isRunning()) or \
                          (self.video_loader_worker and self.video_loader_worker.isRunning())
                if is_busy:
    ...
    def start_batch_processing(self):
        ...
        all_media_widgets = []
        for i in range(self.targetVideosList.count()):
            item = self.targetVideosList.item(i)
            widget = self.targetVideosList.itemWidget(item)
            if widget:
                all_media_widgets.append(widget)

        if not all_media_widgets:
            if not self.clipboard_file_queue:
                 common_widget_actions.create_and_show_messagebox(
                    self, "No Items Found", "There are no items in the target list to process.", self)
            return
        
        self.initialize_and_start_batch(all_media_widgets)

    def initialize_and_start_batch(self, media_widgets_to_process):
        # We start by trying to initialize with the first item in the list.
    ...
    def check_init_status(self, media_widgets_to_process, current_index):
        if current_index >= len(media_widgets_to_process):
            # We've gone through all items and found no faces.
        ...
        # This timer gives the UI event loop time to process the click and run face detection.
        QTimer.singleShot(100, lambda: self.verify_init(media_widgets_to_process, current_index))

    def verify_init(self, media_widgets_to_process, current_index):
        if self.target_faces:
            # Success! We found faces. The UI is now initialized with valid parameters.
            ...
            # Failure. The last item had no faces. We recursively call to try the next one.
    ...
    def cleanup_after_batch(self):
        ...
        if self.control.get('ClearTargetAfterProcessingToggle', False):
            ...
            for item in self.items_in_current_batch:
                widget = self.targetVideosList.itemWidget(item)
                if widget:
                    if self.selected_video_button == widget:
                        self.selected_video_button = None
                    if widget.media_id in self.target_videos:
                        self.target_videos.pop(widget.media_id)
                    row = self.targetVideosList.row(item)
                    if row != -1:
                        self.targetVideosList.takeItem(row)
    ...
    def toggle_embedding_sort(self):
        """Cycles through embedding sort modes and reorders the list."""
        modes = ["Off", "A-Z", "Z-A"]
        
        # If we are currently in "Off" mode, this is the moment to take a backup
        # of the current order before we apply a new sort.
        ...
        try:
            ...
            current_index = -1 # Default to first mode if something is wrong
        
        next_index = (current_index + 1) % len(modes)
        self.embedding_sort_mode = modes[next_index]
        
        # Update button text and trigger the reorder action
        self.sortEmbeddingsButton.setText(f"Sorting: {self.embedding_sort_mode}")
