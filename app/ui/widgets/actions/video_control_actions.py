...
def save_current_frame_to_file(main_window: 'MainWindow'):
    ...
    if output_to_target:
        if main_window.selected_video_button and main_window.selected_video_button.media_path:
            output_folder = os.path.dirname(main_window.selected_video_button.media_path)
        else:
            common_widget_actions.create_and_show_messagebox(main_window, 'Cannot Determine Target Location', "No target media selected to determine output path.", main_window)
            return
    else:
        output_folder = main_window.outputFolderLineEdit.text()

    if not output_folder:
        common_widget_actions.create_and_show_messagebox(main_window, 'No Output Folder Selected','Please select an Output folder in Settings, or enable "Output to Target Location".', main_window)
        return
        
    frame = main_window.video_processor.current_frame.copy()
    if isinstance(frame, numpy.ndarray):
        save_filename = misc_helpers.get_output_file_path(main_window.video_processor.media_path, output_folder, media_type='image')
        ...
        if save_filename:
            try:
                ...
                # Save as JPEG with high quality. You can adjust the quality as needed (1-95).
                ...
                common_widget_actions.create_and_show_messagebox(main_window, 'Error Saving Image', f"An error occurred while saving the image:\n{e}", parent_widget=main_window.saveImageButton)
